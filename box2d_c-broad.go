...
func b2UpdateBroadPhasePairs(tls *_Stack, world uintptr) {
	var alloc, bp, pair, result, shapeA, shapeB, temp, userPairTask, v2, v4 uintptr
	var i, minRange, moveCount, shapeIdA, shapeIdB int32
	_, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = alloc, bp, i, minRange, moveCount, pair, result, shapeA, shapeB, shapeIdA, shapeIdB, temp, userPairTask, v2, v4
	bp = world + 40
	...
	(*b2BroadPhase)(unsafe.Pointer(bp)).MoveResults = b2AllocateArenaItem(tls, alloc, int32FromUint64(uint64FromInt32(moveCount)*uint64(8)), __ccgo_ts)
	(*b2BroadPhase)(unsafe.Pointer(bp)).MovePairCapacity = int32(16) * moveCount
	(*b2BroadPhase)(unsafe.Pointer(bp)).MovePairs = b2AllocateArenaItem(tls, alloc, int32FromUint64(uint64FromInt32((*b2BroadPhase)(unsafe.Pointer(bp)).MovePairCapacity)*uint64(24)), __ccgo_ts+13)
	atomicStoreNInt32(bp+268, 0, int32FromInt32(__ATOMIC_SEQ_CST))
	minRange = int32(64)
	...
	for {
		...
		for pair != uintptrFromInt32(0) {
			...
			v2 = (*b2ShapeArray)(unsafe.Pointer(world+1248)).Data + uintptr(shapeIdA)*288
			goto _3
		_3:
			// if (s_file != NULL)
			//{
			...
			// }
			shapeA = v2
			v4 = (*b2ShapeArray)(unsafe.Pointer(world+1248)).Data + uintptr(shapeIdB)*288
			goto _5
		_5:
			shapeB = v4
